    ...
    while getopts ":hlsdn:x:p:e:" opt; do
        case $opt in
            ...
            \?)
                ...
                ;;
            x)
                _SCRIPT_PATH="$OPTARG" && echo $_SCRIPT_PATH | sed 's/.$//'
                ;;
        esac
    done
}
...
download_media_html() {
    # $1: media link
    "$_CURL" -sS "${_HOST}${1}" > "$_TMP_DIR/$_MEDIA_HTML"
}
...
search_media_by_name() {
    ...
    true > "$_TMP_DIR/$_SEARCH_LIST_FILE"
    for i in $(seq 1 "$len"); do
        ...
        echo "[$l][$lb] $n" | tee -a "$_TMP_DIR/$_SEARCH_LIST_FILE"
    done
}
...
download_episode() {
    # $1: episode number
    local l
    #l=$(grep "\[$1\] " "$_SCRIPT_PATH/$_MEDIA_NAME/$_EPISODE_LINK_LIST" \ ORIGINAL
    l=$(grep "\[$1\] " "$_TMP_DIR/$_EPISODE_LINK_LIST" \
        | awk -F '] ' '{print $2}')
...
create_episode_list() {
    ...
    sf="$_TMP_DIR/$_SOURCE_FILE"
    el="$_TMP_DIR/$_EPISODE_LINK_LIST"
    et="$_TMP_DIR/$_EPISODE_TITLE_LIST"
...
main() {
    ...
    [[ ! -s "$_TMP_DIR/$_SEARCH_LIST_FILE" ]] && print_error "$_SEARCH_LIST_FILE not found. Please run \`-n <name>\` to generate it."
    _MEDIA_NAME=$(sort -u "$_TMP_DIR/$_SEARCH_LIST_FILE" \
                | grep "$_MEDIA_PATH" \
                | awk -F '] ' '{print $2}' \
                | sed -E 's/\//_/g' \
                | tr -d ':')
